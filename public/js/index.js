...
document.addEventListener('DOMContentLoaded', function () {
  var activateRange = function activateRange() {
    var ranges = document.querySelectorAll('.js-range');
    if (!ranges) return;
    ranges.forEach(function (range) {
      var rangeSlider = range.querySelector('.js-range__slider');
      if (!rangeSlider) return;
      var rangeInputs = range.querySelectorAll('.js-range__input');
      if (!rangeInputs) return;
      var inputs = rangeInputs.length > 1 ? [rangeInputs[0], rangeInputs[1]] : [rangeInputs[0]];
      var start = rangeInputs.length > 1 ? [inputs[0].min, inputs[0].max] : inputs[0].min;
      ...
      var setRangeSliderValue = function setRangeSliderValue(i, value) {
        var arr = [null, null];
        arr[i] = value;
...
document.addEventListener('DOMContentLoaded', function () {
  var activateFilter = function activateFilter() {
    ...
    filters.forEach(function (filter) {
      var filterBlocks = filter.querySelectorAll('.js-filter__block');
      filterBlocks.forEach(function (filterBlock) {
        var filterHead = filterBlock.querySelector('.js-filter__head');
        var filterBody = filterBlock.querySelector('.js-filter__body');
